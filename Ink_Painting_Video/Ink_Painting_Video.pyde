...
#5 - Source without Gradient, 6 - Source with Palette, 7 - Source with Palette and not Random
COLOR_MODE = 5
PALETTE = [color(255,196,33), color(138,12,12), color(12,21,138), color(0)]
...
def setup():
    ...
    if backgroundSource != None:
        img = loadImage(backgroundSource)
        image(img, 0, 0)
    else: 
        background(0)
            
def loadNewSource(num):
    global SOURCE_IMG
    numStr = str(num)
    if num < 10:
        numStr = "000" + numStr
        ...
        numStr = "0" + numStr
    #print(numStr)
    SOURCE_IMG = loadImage("SourceVideo/3/" + numStr + ".png")
    
def draw():
    global NUM_FRAMES, FRAME
    # background(0)
    # rTot = 0.0
    # gTot = 0.0
    # bTot = 0.0
    ...
    #         col = SOURCE_IMG.get(x,y)
    #         rTot += red(col)
    #         gTot += green(col)
    #         bTot += blue(col)
    # rAvg = rTot/(width*height)
    # gAvg = gTot/(width*height)
    # bAvg = bTot/(width*height)
    # background(color(rAvg,gAvg,bAvg))
